...
Процессоры позволяют нам задать значение ввиде типа, которые нельзя представить в виде строки стандартными методами. Мы можем передать метод изменения данных дату ^ct.modify[$.updatedAt[^date::now[]]] и класс автоматически вызовет метод sql-date перед передачей в базу данных. Процессоры с перфиксом auto_ «умеют» подставлять осмысленные данные, если не задано значение поля.
...
Иногда удобно доставать вместе с основными полями еще и некоторые поля из связанных таблиц. Давайте немного расширим класс managersTable:
...
Помимо получения записей из таблицы требуется уметь получать обобщенные данне о записях. В SQL'е для этого используются агрегатные функции и класс pfSQLTable предоставляет несколько функций для простого построения запросов с группировкой.
...
Можно задать любую групирующую функцию, поддерживаемую СУБД. Имя поля в результирующих таблице или хеше можно задать через оператор as (если алиас не указать, то именем поля будет все выражение). Несколько примеров:
    ...
    # Количество новых клиентов, сгруппированых по дате.
